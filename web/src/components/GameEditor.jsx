    ...
    <div
      ...
      onKeyDown={({ key }) => {
        if (editorSettings.placeMode) {
          ...
          const featuredIndex = level.data.findIndex(
            (e) => e[0] === featuredId,
          );
          switch (key) {
            case "e":
            ...
            case "p":
              ...
              break;
            case " ":
              if (running) playerRef.current.audio.current.pause();
                ...
                switch (key) {
                  ...
                  case "r":
                    setLevel((l) => {
                      return {
                        ...l,
                        data: l.data.filter((e) => e[0] !== featuredId),
                      };
                    });
                  ...
                  case "w":
                    setLevel((l) => {
                      if (l.data[featuredIndex][3] > 0)
                        l.data[featuredIndex][3]--;
                      return { ...l };
                    });
                    break;
                  case "s":
                    setLevel((l) => {
                      if (l.data[featuredIndex][3] < 2)
                        l.data[featuredIndex][3]++;
                      return { ...l };
                    });
                    break;
                  case "z":
                    ...
                    if (editorSettings.blockMoveStep > 0.1)
                      setSettings({
                        ...editorSettings,
                        blockMoveStep: round(
                          editorSettings.blockMoveStep - 0.1,
                          2,
                        ),
                      });
                      ...
                      setSettings({
                        ...editorSettings,
                        blockMoveStep: round(
                          editorSettings.blockMoveStep - 0.01,
                          2,
                        ),
                      });
                    break;
    ...
    >
        ...
        {level.data.map((t, i) => (
          <EditorType
            ...
            onMoveBlock={(forward) => {
              setLevel((l) => {
                l.data[i][1] = forward
                  ? round(l.data[i][1] + editorSettings.blockMoveStep, 2)
                  : round(l.data[i][1] - editorSettings.blockMoveStep, 2);
                l.data.sort((x, y) => x[1] - y[1]);
                return { ...l };
              });
            ...
            onDeleteBlock={() => {
              setLevel((l) => {
                return {
                  data: [...l.data.slice(0, i), ...l.data.slice(i + 1)],
                  maxId: l.maxId,
                };
              });
              setFeaturedIndex(-1);
      ...
      >
        ...
        <div style={{ textAlign: "left" }}>
          <p hidden={editorSettings.placeMode}>
            <span style={{ fontStyle: "italic" }}>Step Size: </span>
          ...
          <p>
            <span style={{ fontStyle: "italic" }}>Editor Mode: </span>
            {editorSettings.placeMode ? "Place" : "Mover"}
          </p>
          <h3 style={{ padding: "0", margin: "0" }}>Controls</h3>
          {editorSettings.placeMode ? (
            <ul style={{ marginTop: "0" }}>
              <li>
                <span style={{ fontStyle: "italic" }}>Escape: </span> Mover Mode
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>[Any Key]: </span> Place
                Block
              </li>
            </ul>
          ) : (
            <ul style={{ marginTop: "0" }}>
              <li>
                <span style={{ fontStyle: "italic" }}>p: </span> Place Mode
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>r: </span> Delete Block
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>q/e: </span> Select
                left/right
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>a/d: </span> Move
                left/right
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>w/s: </span> Move up/down
              </li>
              <li>
                <span style={{ fontStyle: "italic" }}>z/c: </span> Step size
                up/down
              </li>
            </ul>
          )}
      ...
      <div>
        <button
          style={{ width: "125px" }}
          onClick={() => onPublish(level.data)}
        >
          {props.isPublished ? "Unpublish" : "Publish"}
        </button>
        <button style={{ width: "125px" }} onClick={() => onSave(level.data)}>
          Save
        </button>
      </div>
    </div>
